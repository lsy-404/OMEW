...
| 121 | [Feature] 专属实例固定 medium5 据点 | 为星尘站专属 Worker 定制单一 `medium5` 据点，隐藏目录、创建与切换并锁定服务端访问范围 | 星尘站专属服务器不应暴露多据点模型 | 🔄 进行中 |
| 122 | [Feature] 部署级品牌与能力开关 | 增加可部署自定义 logo、表情和反应开关，并让前后端共同遵守实例配置 | 不同 OMEW 实例需要自定义品牌与能力边界 | 🔄 进行中 |
